  ...
  "AuctionActor" should {
    ...
    "add lot" in {
      ...
      withSession { implicit testSession: TestSession =>
        ObjectTransaction[CreateAuction](AuctionActor, "CreateAuction", "CreatedAuction").run
        ObjectTransaction[CreatedAuction](LotActor, "CreatedAuction", "CreatedAuctionValidated").run
        FunctionalTransaction[CreatedAuctionValidated]({ a: CreatedAuctionValidated =>
          Future.successful(Done("hi"))
        }, "CreatedAuctionValidated", "End").run

        kafkaMockRequirements.publishToKafka("CreateAuction", CreateAuction.serialize(CreateAuction.example))
        kafkaMockRequirements.publishToKafka("CreateAuction", CreateAuction.serialize(CreateAuction.example))
